...
export function Profile() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="个人资料"
        description="查看当前浏览器会话对应的账号与授权范围。"
      />
      <Tabs defaultValue="profile" className="space-y-5">
        ...
        <TabsContent value="profile" className="space-y-6">
          <Card className="max-w-3xl">
            <dl className="grid gap-5 sm:grid-cols-2">
              ...
              <Detail
                label="用户 ID"
                value={
                  <code className="break-all text-[length:var(--text-caption)]">
                    {user?.id || "-"}
                  </code>
                }
              />
          ...
          <div className="space-y-3">
            <h2 className="text-[length:var(--text-heading-sm)] font-semibold text-[color:var(--foreground)]">
              当前权限
            </h2>
...
function Detail({ label, value }: { label: string; value: ReactNode }) {
  return (
    <div className="space-y-1">
      <dt className="text-[length:var(--text-label)] text-[color:var(--muted-foreground)]">
        {label}
      </dt>
      <dd className="text-[length:var(--text-body-sm)] text-[color:var(--foreground)]">
        {value}
      </dd>
    </div>
  );
