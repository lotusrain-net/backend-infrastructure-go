...
describe("public ApiClient", () => {
  it("uses an injected base URL and unwraps the stable envelope", async () => {
    ...
    await expect(client.request<{ id: string }>("/items/1")).resolves.toEqual({
      id: "1",
    });
  ...
  it("shares refresh work and expires the session after one retry", async () => {
    ...
    const fetcher = vi.fn<typeof fetch>().mockResolvedValue(
      new Response(
        JSON.stringify({ code: 401, msg: "unauthorized", data: {} }),
        {
          status: 401,
          headers: { "content-type": "application/json" },
        },
  ...
  it("passes typed-array request bodies through without JSON encoding", async () => {
    const fetch = vi.fn(
      async (_input: RequestInfo | URL, init?: RequestInit) => {
        ...
        return new Response(
          JSON.stringify({ code: 200, msg: "success", data: null }),
          {
            status: 200,
            headers: { "content-type": "application/json" },
          },
...
describe("withQuery", () => {
  it("appends arrays and preserves an existing query", () => {
    expect(
      withQuery("/items?active=true", { tag: ["a", "b"], page: 2, empty: "" }),
    ).toBe("/items?active=true&tag=a&tag=b&page=2");
  });
});

it("preserves Retry-After for a throttled request", async () => {
  const client = new ApiClient({
    fetch: vi
      .fn()
      .mockResolvedValue(
        new Response(
          JSON.stringify({ code: 429, msg: "too many requests", data: null }),
          {
            status: 429,
            headers: {
  ...
  await expect(client.request("/auth/email-code")).rejects.toMatchObject({
    status: 429,
