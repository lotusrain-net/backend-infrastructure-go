...
```sh
go get github.com/lotusrain-net/backend-infrastructure-go@v0.2.0
go mod tidy
go mod vendor
```

Replace the old module prefix in imports with
`github.com/lotusrain-net/backend-infrastructure-go`. The `web/go.mod` file is
a separate nested module at `/web`; it is not released as a Go submodule.
...
Repository owners publish and fetch the Go module over SSH:

```sh
git remote set-url origin git@github.com:lotusrain-net/backend-infrastructure-go.git
ssh -T git@github.com
git fetch --tags
```

Developers configure the namespace and map HTTPS GitHub module fetches to SSH
without embedding credentials in module metadata:

```sh
go env -w GOPRIVATE=github.com/lotusrain-net/*
git config --global url."ssh://git@github.com/".insteadOf https://github.com/
```

CI must use a read-only deploy key, GitHub App, or organization-scoped
credential for this module. Never put a personal access token in a Git URL,
script, `go.mod`, or committed configuration.
...
```sh
npm install @lotusrain-net/backend-infrastructure-web@0.2.0
```

Use `@lotusrain-net/backend-infrastructure-web/api`, `/theme`, `/ui`,
