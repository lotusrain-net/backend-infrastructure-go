...
个人安全策略 `mode` 为 `default`、`email`、`totp`。`default` 不要求邮箱码；`email` 仅适用于已验证邮箱，与注册邮箱验证开关无关。启用 `email` 时，修改邮箱返回 `403`，原邮箱与验证时间保持不变；同邮箱的资料修改不受影响。用户需先将安全策略改为 `default` 才能修改邮箱。其他模式下修改邮箱会清空验证时间；修改后可在个人资料页重新验证邮箱，再启用 `email`。邮箱修改与邮箱策略启用在数据库中串行校验，避免并发操作使账号无法登录。
...
个人资料页展示邮箱验证状态。选择“启用邮箱二次验证”时，未验证账号先发送验证码，输入六位验证码后点击“验证并启用邮箱二次验证”。已验证账号直接保存策略；已启用 TOTP 的账号仍需先用当前密码及 TOTP/恢复码停用。

- `POST /api/v1/users/me/email/verification-code`：需要登录，只向当前账号邮箱发送，不接受客户端指定收件人。返回 `202 {sent:true,resend_after_seconds:60}`。
- `POST /api/v1/users/me/email/verify`：请求 `{email_code:"123456"}`，验证成功返回包含 `email_verified_at` 的用户资料。然后通过既有 `PUT /api/v1/users/me/security {mode:"email"}` 启用二次验证。

新验证码使用独立的 `verify_email` 用途，注册和登录码不可混用。有效期十分钟、重发冷却六十秒，沿用每邮箱/IP 限流和五次错误上限。验证码消费记录与邮箱验证状态在同一 PostgreSQL 事务中提交；事务重读并锁定账号，邮箱已变更返回 409，事务失败可重试，成功后不可重放，即使 Redis 清理失败也不会重复接受。验证本身不修改 TOTP 或安全策略。

未验证邮箱启用策略返回 403 `email not verified`；验证码错误、过期或重复使用返回 422 `data.email_code`，不会触发会话刷新。邮件传输不可用或拒收返回 503。前端 API 层统一提供中文错误提示，保留原始 `details` 用于协议判断；429 继续读取 `Retry-After` 显示重发倒计时。
