...
A release publishes the Go module and
`@lotusrain-net/backend-infrastructure-web` from the same commit. The workflow
accepts a semantic version tag (for example `v0.2.0`) and must be dispatched
from the `main` branch.

## One-time repository setup

- Configure the GitHub `release` environment with required reviewers.
- Protect each release tag (for example `v0.2.0`) with a repository ruleset
  that prevents updates and deletion while allowing the release workflow to
  create it.
...
| Setting | Value |
| --- | --- |
| Organization or user | `lotusrain-net` |
| Repository | `backend-infrastructure-go` |
| Workflow filename | `release.yml` |
| Environment | `release` |
| Allowed action | `npm publish` |

npm trusted publishers are configured from an existing package's settings. A
newly renamed scope such as `@lotusrain-net/backend-infrastructure-web` is a new
package name, so its first verified tarball must be published once by the owner
interactively; then configure the table above and use only OIDC for subsequent
releases. Do not store that interactive credential in the repository or GitHub
Actions. The package manifest's `repository.url` must exactly match the GitHub
...
npm publish lotusrain-net-backend-infrastructure-web-0.2.0.tgz --access public
```
...
CI passed, and that the npm package version matches the dispatch version
(for example, package version `0.2.0` and input `v0.2.0`). Dispatch the
`Release` workflow with the semantic version tag.
...
3. It retains the real npm tarball installed by both consumer fixtures,
   derives its name from the package manifest, records its source commit and
   SHA-256 digest, and transfers that exact artifact between jobs.
4. It creates an annotated tag for the triggering commit and verifies its
   peeled remote target before any npm publication.
...
For the first publication of a package name, the `bootstrap` job intentionally
stops after tag creation and artifact verification and prints the owner-only
command above. No token is accepted by the workflow. Once the owner publishes
that artifact and configures Trusted Publishing, rerunning verifies the
existing tarball and completes the GitHub Release. The next unpublished npm
version exercises the OIDC publisher; private source repositories do not
receive npm provenance.

After completion, independently verify:

```sh
GOWORK=off go list -m github.com/lotusrain-net/backend-infrastructure-go@v0.2.0
npm view @lotusrain-net/backend-infrastructure-web@0.2.0 version dist.integrity
```
