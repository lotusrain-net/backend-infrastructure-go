...
func TestParseLevelRequiresExactLowercaseValue(t *testing.T) {
	for _, value := range []string{" INFO", "info ", "INFO"} {
		if _, err := ParseLevel(value); err == nil {
			t.Fatalf("ParseLevel(%q) error = nil, want validation error", value)
		}
	}
}
