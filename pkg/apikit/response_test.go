...
func TestWriteEncodesStableEnvelope(t *testing.T) {
	...
	apikit.Write(recorder, http.StatusCreated, map[string]string{"id": "42"})
...
func TestWriteErrorUsesPublicApplicationError(t *testing.T) {
	recorder := httptest.NewRecorder()
	apikit.WriteError(recorder, apikit.NotFound("user"))
...
func TestWriteValidationErrorPlacesDetailsInData(t *testing.T) {
	recorder := httptest.NewRecorder()
	apikit.WriteError(recorder, apikit.Validation(map[string]string{"email": "is required"}))
...
func TestWriteBuffersBeforeCommittingStatusAndFallsBackOnEncodingFailure(t *testing.T) {
	recorder := httptest.NewRecorder()
	apikit.Write(recorder, http.StatusCreated, make(chan int))
...
func TestWriteErrorDropsUnencodableErrorData(t *testing.T) {
	recorder := httptest.NewRecorder()
	err := apikit.New(51000, "unsafe payload", http.StatusBadGateway, nil)
	err.Data = make(chan int)

	apikit.WriteError(recorder, err)
...
func TestWriteErrorDoesNotLeakUnknownError(t *testing.T) {
	recorder := httptest.NewRecorder()
	apikit.WriteError(recorder, errors.New("secret database message"))
